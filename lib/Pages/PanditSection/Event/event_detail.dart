...
class _EventDetailState extends State<EventDetail> {
                ...
                children: [
                    ...
                    children: [
                      SizedBox(
                          ...
                          child: ResponsiveWidget.isSmallScreen(context)
                              ? Image.network(
                                  "${events[int.parse(indx)]["image"]}")
                              : Image.network(
                                  "${events[int.parse(indx)]["bigS"]}",
                                  fit: BoxFit.fitWidth,
                                )),
                      Padding(
                  ...
                  Padding(
                    padding: ResponsiveWidget.isSmallScreen(context)
                        ? EdgeInsets.all(15)
                        : EdgeInsets.only(
                            top: 15, left: width * 0.1, right: width * 0.1),
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            SizedBox(width: 5),
                            Text1(
                              data: "${events[int.parse(indx)]["place"]}",
                              max: 14,
                              min: 12,
                              clr: Colors.black54,
                            ),
                            Spacer(),
                            Text1(
                              data: "${events[int.parse(indx)]["duration"]}",
                              max: 14,
                              min: 12,
                              clr: Colors.black54,
                            )
                        ...
                        Row(
                          children: [
                            Text1(
                              data:
                                  "${events[int.parse(indx)]["participants"]!.length} Participants",
                              max: 14,
                              min: 12,
                              clr: Colors.black54,
                            ),
                            Spacer(),
                            Text1(
                              data: "${events[int.parse(indx)]["total_days"]}",
                              max: 14,
                              min: 12,
                              clr: Colors.black54,
                            )
                        ...
                        authController.user == null
                            ...
                            : Row(
                                ...
                                children: [
                                  buttonFocus(
                                      events[int.parse(indx)]["status"], events)
                                ],
                              ),
                        SizedBox(
                          height: 20,
                        ...
                        Text1(
                          ...
                          max: 24,
                          min: 20,
                          clr: Color(0xff181c2c),
                        ),
                        SizedBox(
                          height: 20,
                        ),
                        Text1(
                          data: "${events[int.parse(indx)]["about"]}",
                          max: 14,
                          min: 12,
                          clr: Colors.black54,
                        ),
                  ...
                  Padding(
                      padding: ResponsiveWidget.isSmallScreen(context)
                          ...
                          : EdgeInsets.only(
                              top: 15, left: width * 0.1, right: width * 0.1),
                      child: bottomSection(
                          events[int.parse(indx)]["status"], events, context)),
                ],
              );
            }),
      ),
  ...
  Widget bottomSection(
      String status, List<dynamic> events, BuildContext context) {
    double height = MediaQuery.of(context).size.height;
    if (status == "upcoming") {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
          ...
          ListView.builder(
              ...
              itemBuilder: (_, index) {
                return Padding(
                  ...
                  child: Text1(
                    data: "${events[int.parse(indx)]["terms"]![index]}",
                    max: 14,
                    min: 12,
                    clr: Colors.black54,
                  ),
      ...
      return Column(
        children: [
          ...
          StreamBuilder<QuerySnapshot>(
              stream: FirebaseFirestore.instance
                  .collection(
                      "PujaPurohitFiles/events/${events[int.parse(indx)]["name"]}")
                  .orderBy("votes")
                  .snapshots(),
              builder: (context, snapshot) {
                if (snapshot.data == null) {
                ...
                return ResponsiveWidget.isSmallScreen(context)
                    ...
                    : GridView.builder(
                        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                          crossAxisCount:
                              ResponsiveWidget.isMediumScreen(context) ? 2 : 4,
                          crossAxisSpacing: 10,
                          mainAxisSpacing: 10,
                          childAspectRatio:
          ...
          child: ListView.builder(
              ...
              itemBuilder: (_, index) {
                return InkWell(
                  onTap: () {
                    Get.toNamed(
                        '/imageview?id=${events[int.parse(indx)]["top"]![index]["id"]}&vote=false&name=${events[int.parse(indx)]["name"]}');
                  },
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                        image: DecorationImage(
                            image: NetworkImage(
                                "${events[int.parse(indx)]["top3"]![index]["image"]}"),
                            fit: BoxFit.fill)),
                  ),
                );
              }),
      ...
      children: [
        ...
        Container(
          ...
          child: InkWell(
              onTap: () {
                Get.toNamed(
                    '/imageview?id=${snaps[index]["id"]}&vote=${snaps[index]["vote"]}&name=${snaps[index]["event"]}');
              },
              child: Image.network(
                "${snaps[index]["image"]}",
                fit: BoxFit.fill,
              ...
              StreamBuilder<DocumentSnapshot>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    return Padding(
                      ...
                      child: Row(
                        children: [
                          snaps[index]["voters"].contains(
                                  '${authController.user!.phoneNumber}')
                              ? InkWell(
                                  onTap: () {
                                    List<dynamic> voters = snapshot.data!
                                        .get("${snaps[index]['event']}V");
                                    List<dynamic> localVoters =
                                        snaps[index]['voters'];
                                    ...
                                    FirebaseFirestore.instance
                                        .doc(
                                            "PujaPurohitFiles/events/${snaps[index]['event']}/${snaps[index]['id']}")
                                        .update({
                                      'voters': totalL,
                          ...
                          Container(
                              ...
                              child: Text1(
                                data: "${snaps[index]["voters"].length} votes",
                                max: 14,
                                min: 12,
                                clr: Colors.white,
                                weight: FontWeight.bold,
                          ...
                          Spacer(),
                          Obx(() {
                            return loadController.load.value.active
                                ? SizedBox(
                                    height: 50,
                                    width: 50,
                                    child: Loader(),
                                ...
                                : InkWell(
                                    onTap: () async {
                                      Share.share(
                                          'Visit the link and vote ${snaps[index]["name"]} at Puja Purohit pujapurohit.in/#/imageview?id=${snaps[index]["id"]}&vote=${snaps[index]["vote"]}&name=${snaps[index]["event"]}',
                                          subject: 'Puja Purohit Contest');
                                    },
    ...
    return StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          List<dynamic> participants =
              snapshot.data!.get("${events[int.parse(indx)]["name"]}P");
          return InkWell(
            onTap: () {
              // eventController.updateparticipants(participants);
              if (participants.contains("${authController.user!.uid}")) {
                Get.snackbar("Info", "Already Participated",
                    backgroundColor: Colors.white);
                Get.toNamed(
                    '/imageview?id=${authController.user!.uid}&vote=false&name=${events[int.parse(indx)]["name"]}');
              } else {
            ...
            child: Container(
              decoration: BoxDecoration(
                  color: participants.contains("${authController.user!.uid}")
                      ? Color(0xff181c2c)
                      : Colors.white,
                  boxShadow: [
                    BoxShadow(
                        color:
                            participants.contains("${authController.user!.uid}")
                                ? Color(0xff181c2c).withOpacity(0.2)
              ...
              padding: EdgeInsets.all(15),
              child: Text1(
                data: participants.contains("${authController.user!.uid}")
                    ? "View Participant"
                    : "Participate Now",
                max: 14,
                min: 12,
                clr: participants.contains("${authController.user!.uid}")
                    ? Colors.white
                    : Color(0xff181c2c),
              ),
            ),
        ...
        return GetX<PanditKeywordController>(
          ...
          builder: (PanditKeywordController panditController) {
            if (panditController != null && panditController.pandits != null) {
                      ...
                      child: new ListView.builder(
                        ...
                        itemBuilder: (BuildContext ctxt, int index) {
                          return new PanditNewCard(
                              ...
                              lat: double.parse(
                                  '${userController.userModel.value.lat}'),
                              lng: double.parse(
                                  '${userController.userModel.value.lng}'));
                        },
                      ),
                    ),
                  ),
              ...
              return Center(
                  child: SizedBox(height: 50, width: 50, child: Loader()));
            }
          },
        );
      },
